...
"""
    File name: rate_max_vs_noise_random_networks.py
    Author: Giacomo Baggio
    Email: baggio.giacomo@gmail.com
    Description: Compute and Plot Max Shannon Transmission Rate vs. Noise Level for Random Network Topologies
...
for sigma2 in sigma2_vec: 
    ...
    # compute effective rank Sigma^*
    eigs, eigvs = np.linalg.eig(Sigma_opt_norm)
    eff_rank = np.square(np.sum(np.real(eigs)))/np.sum(np.square(np.real(eigs)))
